...
import com.spring.weather.domain.AjaxWriteList;
import com.spring.weather.domain.AjaxWriteResult;
import com.spring.weather.domain.WriteDTO;
import com.spring.weather.service.AjaxService;
...
public class AjaxController {
	...
	// A. 글 목록(페이징)
		...
		// C. 글 작성
		@PostMapping("")
		public AjaxWriteResult wirteOk(WriteDTO dto) {
			int count = 0;
			String datetime=null;
			// response 에 필요한 값들
			StringBuffer message = new StringBuffer();
		...
		// F. 글 삭제
		@DeleteMapping("")
		public AjaxWriteResult deleteOk(int [] w_uid) {
			int count = 0;
			...
			try {	
				if(w_uid == null) {
					...
					count = ajaxService.deleteByUid(w_uid);
				status = "OK";
